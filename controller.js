...
angular.module('SmartMirror')
	.controller('MyCovid19', function ($scope, $translate,$timeout, _MyChartService,SpeechService, Focus){

	var Covid19_charts={}
	var charts={}
	var attribution_label={'countries':'European Centre for Disease Prevention and Control','states':'NY Times'};
  $scope.MyCovid19={config: config.mycovid19,attributions:attribution_label}
...
console.log("start found canvas for chart="+ chart.chartname+" id="+canvas.id)	
			...
			// Create gradient
			let gradient = ctx[canvas.id].createLinearGradient(0, 0, parseInt(chart.chartwidth), 0);
			...
			ctx[canvas.id].fillText("Big smile!", parseInt(chart.chartwidth)/2, parseInt(chart.chartheight)/2);
			console.log("done  found canvas for chart="+ chart.chartname+" id="+canvas.id)	
			chart.id=canvas.id
			...
			chart['config']={}
			chart.config.type= (chart.countries!== undefined) ?'countries':'states'			
    ...
    if(_chart_debug)
      console.log(payload.id+" refreshing "+ moment().format())
        ...
        // each instance (this/self) is ONE report of multiple lines of the same location type and data type, cases or deaths
 				for(payload.x[payload.id]=0 ; payload.x[payload.id]<payload.config[payload.config.type].length;payload.x[payload.id]++){
          var location = payload.config[payload.config.type][payload.x[payload.id]]         
          ...
          if(payload.data != undefined){            
             ...
             __$ds[payload.id][payload.config.type].push({
                   ...
                   fill: false,
                   borderColor: payload.config.colorsforsource[payload.x[payload.id]], // Add custom color border (Line)
                   backgroundColor: payload.config.colorsforsource[payload.x[payload.id]],
  ...
  //      var info = {  ourID:canvas.id ,canvas:canvas,  data:__$ds, config: payload.config, ticklabel:payload.ticklabel }
        ...
        //if(lastMoment.format('MM/DD/YYYY') == currentMoment_date || self.displayedOnce==false){          
          ...
          payload.ticklabel[payload.id].push(first_country_data['cases'].slice(1)[0].x)            
